...
for(my$i=0;$i<$str_num;$i++){
    ...
    if($symble0[6] eq ""){
      print OUT_3  "$symble0[0]  $symble0[3] ";#test.txt
      }
     elsif($symble0[10] eq ""){
       print OUT_3  "$symble0[0]  $symble0[5] ";
     }
    #negative numbers
     my$num=$ne_num[$i];
  # print OUT_1 "   num:$num\n";
...
##########control the output num#############
   ...
   ##################pos is the str posi ; pos1 is the pre posi########## 
               ...
               if($sign<2){
                    print OUT_3 "$symble3[1]   1";#test1_negative1.txt
                    }
               if($sign<6){  
                 print OUT_1 "\nthe 5'arm mature miRNA position is  $pos1 : $string1";
                 
                     }
               if($sign<6){
                     my$devi=$pos1-$symble0[1];
      ...
      print OUT_1 "\n";
       print OUT_2 "$symble0[1]  $symble0[2] $symble0[3] $symble0[4] $symble0[5] $symble0[6] $symble0[7] $symble0[8]   $symble0[9] $symble0[10] $symble0[11]  $symble0[12]\n";
      print OUT_3 "\n";
     $star=$num+$star;
}    
